...
def main():
    print("=" * 80)
    print("## RAGAS评估协调器实战应用 - 自定义API版本")
    print("支持自定义OpenAI API端点：https://api.chatanywhere.tech/")
    print("=" * 80)
    ...
    try:
        ...
        # 配置LLM和Embeddings，使用自定义API
        评估用LLM = LangchainLLMWrapper(
            ChatOpenAI(
                model="gpt-3.5-turbo",
        ...
        print("✓ 基础模型配置完成（使用自定义API）")
        print(f"  - API Host: {custom_host}")
    ...
    try:
        ...
        # 保存结果到CSV文件
        详细结果表.to_csv('评估结果明细_自定义API.csv', index=False, encoding='utf-8')
        print("\n✓ 详细结果已保存到 '评估结果明细_自定义API.csv'")
        ...
        详细结果表 = pd.DataFrame(模拟结果数据)
        print("\n样本级评估明细（模拟）：")
        print(详细结果表)
        
        # 保存模拟结果
        详细结果表.to_csv('评估结果明细_自定义API_模拟.csv', index=False, encoding='utf-8')
        print("\n✓ 模拟结果已保存到 '评估结果明细_自定义API_模拟.csv'")
    
    print("\n## 评估完成！")
    print("RAGAS评估协调器实战应用（自定义API版本）演示成功完成。")
